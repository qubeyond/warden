...
void CliService::print_report(const std::string& path, const DetectionResult& result) {
    ...
    switch (result.file_type) {
        case warden::common::FileType::MEDIA:      type_str = "MEDIA"; break;
        case warden::common::FileType::ARCHIVE:    type_str = "ARCHIVE"; break;
        case warden::common::FileType::EXECUTABLE: type_str = "EXEC"; break;
        default:                                   type_str = "OTHER"; break;
    }

    std::string v_str;
    if (result.verdict == warden::common::Verdict::MALWARE)      v_str = "[!] MALWARE";
    else if (result.verdict == warden::common::Verdict::SUSPICIOUS) v_str = "[?] SUSPICIOUS";
    else v_str = "[+] SAFE";

    std::cout << std::left  << std::setw(14) << v_str << " | "
              << std::left  << std::setw(8)  << type_str << " | "
              << std::right << std::fixed    << std::setprecision(2) << std::setw(6) << (result.max_probability * 100.0f) << "% | "
              << std::right << std::setw(3)  << result.suspicious_chunks << "/" 
              << std::left  << std::setw(7)  << std::to_string(result.total_chunks) + " chunks" << " | "
              << path << std::endl;
}
